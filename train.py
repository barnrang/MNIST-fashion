...
def load_data():
    ...
    X_train, X_test = [np.reshape(
        _, (-1, 28, 28, 1)) / 255. for _ in [X_train, X_test]]
...
def affine_transform(X, rate):
    ...
    trans_matrix = tf.cond(prob(rate), lambda: rotate(
        trans_matrix), lambda: trans_matrix)
    trans_matrix = tf.cond(prob(rate), lambda: shear(
        trans_matrix), lambda: trans_matrix)
    trans_matrix = tf.cond(prob(rate), lambda: scale(
        trans_matrix), lambda: trans_matrix)
...
def run(model_save=None, model_load='model/siamese2', train_loop=1e5, reg_str=1e-4, lr_str=1e-3, track=0):
    ...
    def model(X, reg_pow=0.0001):
        ...
        with tf.variable_scope('conv1'):
            X = tf.layers.conv2d(X, 64, kernel_size=[5, 5], strides=[
                                 1, 1], activation=tf.nn.relu, kernel_initializer=Winit, bias_initializer=binit, kernel_regularizer=reg)
            X = tf.layers.max_pooling2d(X, [2, 2], 2)
        with tf.variable_scope('conv2'):
            X = tf.layers.conv2d(X, 128, kernel_size=[3, 3], strides=[
                                 1, 1], activation=tf.nn.relu, kernel_initializer=Winit, bias_initializer=binit, kernel_regularizer=reg)
            X = tf.layers.max_pooling2d(X, [2, 2], 2)
            X = tf.contrib.layers.flatten(X)
        with tf.variable_scope('full1'):
            X = tf.layers.dense(X, 1024, kernel_initializer=Denseinit,
                                kernel_regularizer=reg, activation=tf.sigmoid)
    ...
    def combine_predict(X1, X2):
        ...
        diff = tf.abs(X1 - X2)
        out = tf.layers.dense(
            diff, 1, kernel_initializer=Denseinit, use_bias=False)
    ...
    XL, XR = tf.cond(is_training,
                     lambda: (affine_transform(X1, 0.5),
                              affine_transform(X2, 0.5)),
                     lambda: (X1, X2))
    ...
    for i in range(train_loop):
        ...
        for phase in ['train', 'val']:
            if phase == 'train':
                ...
                first, second, expect = data.make_batch(
                    batch_size, dat_type='train')
                feed = {X1: first, X2: second, y: expect, is_training: True}
            else:
                step = [loss, accuracy, correct_pred]
                first, second, expect = data.make_batch(
                    val_size, dat_type='val')
            ...
            if i % 100 == 0:
                print('{} loss is {} and accuracy is {}'.format(
                    phase, current_loss, acc))
    try:
        pbar.close()
    except:
        pass
...
def arg_parse():
    ...
    parser.add_argument("--train_loop", help="Looping count",
                        default=1e5, type=int)
    parser.add_argument(
        "--reg_str", help="regularization strenght", default=1e-4, type=float)
    parser.add_argument("--lr_str", help="learning rate",
                        default=1e-3, type=float)
    parser.add_argument(
        "--track", help="track number to playback", default=0, type=int)
